namespace b {
    ...
    async function main() {
        ...
        // 학습 데이터 읽기
        countOfData = await inputData(dataSets);
        ...
        while (error > LIMIT) {
            ...
            countOfRepetitions++;
            // 오차 출력
            if (printError / error > 1) {
                printError = error;
                console.log(countOfRepetitions, printError);
            }
        }
        // 학습 종료
        console.log("반복 횟수: ", countOfRepetitions);
     ...
     * @param dataSet
     */
    async function inputData(dataSets: Array<Array<number>>): Promise<number> {
